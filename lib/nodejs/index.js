...
function _enqueueSeries(emitter, queue, task, done) {
  request(task.series, 'utf8', function(err) {
    ...
    if (err) return done(err);
    task.seriesName = shared.common.clean(task.series.title); // title
    if (!task.seriesName) return done(new Error('No series: ' + task.address));
    task.seriesPath = path.join(task.options.output || '', task.seriesName); // output path
    task.persistentPath = path.join(task.seriesPath, persistentFilename); // Path for .mrpersistent
    task.persistent = {};
    fs.exists(task.persistentPath, function(exists) {
      ...
      fs.readFile(task.persistentPath, 'utf8', function(err, data) {
        ...
        try {
          if (err) return done(err);
          /*
            '{\n  "298341": "Naruto Gaiden The Seventh Hokage V01 #008.1.cbz",\n  "298719": "Na... (length: 250)'
          */
...
task
 series: 
   { address: undefined,
     ...
     imageAddress: 'http://a.mfcdn.net/store/manga/15684/cover.jpg?1437450860',
     summary: 'A spin-off sequel mini-series of the Naruto and part of the grand master plan of... (length: 172)',
     title: 'Naruto Gaiden: The Seventh Hokage',
     type: 'manga' },
  seriesName: 'Naruto Gaiden The Seventh Hokage',
  seriesPath: '/media/kenpeter/3E3C68780C3F137A/dell_backup_07022012/entertainment/manga/Naruto... (length: 106)',
  persistentPath: '/media/kenpeter/3E3C68780C3F137A/dell_backup_07022012/entertainment/manga/Naruto... (length: 120)',
  persistent: 
   { '298341': 'Naruto Gaiden The Seventh Hokage V01 #008.1.cbz',
     '298719': 'Naruto Gaiden The Seventh Hokage V01 #009.cbz',
     '300371': 'Naruto Gaiden The Seventh Hokage V01 #010.cbz',
     '301781': 'Naruto Gaiden The Seventh Hokage V01 #010.1.cbz' } }
